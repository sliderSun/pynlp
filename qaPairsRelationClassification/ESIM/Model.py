...
from modules import get_token_embeddings, positional_encoding, multihead_attention, label_smoothing, gelu
from utils import print_shape


class ESIM(object):
    def __init__(self, seq_length, n_vocab, embedding_size, hidden_size, attention_size, n_classes, batch_size,
                 learning_rate, optimizer, l2, clip_value, arg):
        # transformer init
        self.arg = arg
        self.arg.n_vocab = n_vocab
        # model init
        self._parameter_init(seq_length, n_vocab, embedding_size, hidden_size, attention_size, n_classes, batch_size,
                             learning_rate, optimizer, l2, clip_value)
    ...
    def _parameter_init(self, seq_length, n_vocab, embedding_size, hidden_size, attention_size, n_classes, batch_size,
                        learning_rate, optimizer, l2, clip_value):
        """
    ...
    def _placeholder_init(self):
        ...
        # self.premise_mask = tf.placeholder(tf.int32, [None, self.seq_length], 'premise_actual_length')
        # self.hypothesis_mask = tf.placeholder(tf.int32, [None, self.seq_length], 'hypothesis_actual_length')
        self.embed_matrix = tf.placeholder(tf.float32, [self.n_vocab, self.embedding_size], 'embed_matrix')
        self.dropout_keep_prob = tf.placeholder(tf.float32, name="dropout_keep_prob")

    # build graph
    def _logits_op(self):
        # a_bar, b_bar = self._inputTransformerEncodingBlock('input_encoding')
    ...
    def _feedForwardBlock(self, inputs, hidden_dims, num_units, scope, isReuse=False, initializer=None):
        ...
        with tf.variable_scope(scope, reuse=isReuse):
            ...
            with tf.variable_scope('feed_foward_layer1'):
                ...
                outputs = tf.layers.dense(inputs, hidden_dims, gelu, kernel_initializer=initializer)
    ...
    def _inputEncodingBlock(self, scope):
        ...
        with tf.device('/cpu:0'):
            self.Embedding = get_token_embeddings(self.n_vocab, self.embedding_size, "embedding", zero_pad=True)
            # 加入位置向量
            self.embeded_left = tf.nn.embedding_lookup(self.Embedding, self.premise)
            self.embeded_left = self._encode(self.embeded_left)

            self.embeded_right = tf.nn.embedding_lookup(self.Embedding, self.hypothesis)
            self.embeded_right = self._encode(self.embeded_right)
    ...
    def _encode(self, enc, training=True):
        ...
        with tf.variable_scope("encoder", reuse=tf.AUTO_REUSE):
            ...
            enc = tf.layers.dropout(enc, self.arg.dropout_rate, training=training)
    ...
    def _loss_op(self, l2_lambda=0.0001):
        with tf.name_scope('cost'):
            ...
            losses = tf.nn.sigmoid_cross_entropy_with_logits(labels=self.y_smoothed, logits=self.logits)
