...
tf.flags.DEFINE_integer("batch_size", 256, "Batch size for training/evaluating.")  # 批处理的大小 32-->128-->512
...
tf.flags.DEFINE_integer("sequence_length", 60, "max sentence length")
...
tf.flags.DEFINE_string("traning_data_path", "../train-zhihu4-only-title-all.txt",
                       "path of traning data.")  # test-zhihu4-only-title-all.txt.one record like this:'w183364 w11 w3484 w3125 w155457 w111 __label__-2086863971949478092'
tf.flags.DEFINE_string("word2vec_model_path", "../zhihu-word2vec-title-desc.bin-512",
                       "word2vec's vocabulary and vectors")  # zhihu-word2vec.bin-100-->zhihu-word2vec-multilabel-minicount15.bin-100
tf.flags.DEFINE_boolean("multi_label_flag", False,
                        "use multi label or single label.")  # set this false. becase we are using it is a sequence of token here.
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        # Instantiate Model
        model = Transformer(FLAGS.num_classes, FLAGS.learning_rate, FLAGS.batch_size, FLAGS.decay_steps,
                            FLAGS.decay_rate, FLAGS.sequence_length,
                            vocab_size, FLAGS.embed_size, FLAGS.d_model, FLAGS.d_k, FLAGS.d_v, FLAGS.h, FLAGS.num_layer,
                            FLAGS.is_training, l2_lambda=FLAGS.l2_lambda)
        # Initialize Save
        ...
        for epoch in range(curr_epoch, FLAGS.num_epochs):
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),
                ...
                if counter % 50 == 0:
                    print("transformer.classification==>Epoch %d\tBatch %d\tTrain Loss:%.3f\tTrain Accuracy:%.3f" % (
                ...
                if FLAGS.batch_size != 0 and (start % (FLAGS.validate_step * FLAGS.batch_size) == 0):
                    ...
                    print("transformer.classification.validation.part. previous_eval_loss:", previous_eval_loss,
                    ...
                    if eval_loss > previous_eval_loss:  # if loss is not decreasing
                        ...
                        print("transformer.classification==>validation.part.learning_rate1:", learning_rate1,
                    ...
                    # print("HierAtten==>Epoch %d Validation Loss:%.3f\tValidation Accuracy: %.3f" % (epoch, eval_loss, eval_acc))
                        ...
                        if eval_loss < best_eval_loss:
                            print("transformer.classification==>going to save the model.eval_loss:", eval_loss,
...
def do_eval(sess, model, evalX, evalY, batch_size, vocabulary_index2word_label, eval_decoder_input=None):
    ...
    for start, end in zip(range(0, number_examples, batch_size), range(batch_size, number_examples, batch_size)):
        ...
        curr_eval_loss, logits, curr_eval_acc, pred = sess.run(
            [model.loss_val, model.logits, model.accuracy, model.predictions],
