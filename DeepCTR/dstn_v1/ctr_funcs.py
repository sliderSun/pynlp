"""
Created on @Time:2019/7/15 10:49
@Author:sliderSun 
@FileName: ctr_funcs.py.py
"""
...
def tf_input_pipeline(file_names, batch_size, num_epochs=1, label_col_idx=0, record_defaults=None):
    # shuffle over files
...
def tf_input_pipeline_test(file_names, batch_size, num_epochs=1, label_col_idx=0, record_defaults=None):
    # shuffle over files
    if record_defaults is None:
        record_defaults = record_defaults
    file_name_queue = tf.train.string_input_producer(file_names, num_epochs=num_epochs, shuffle=True)
