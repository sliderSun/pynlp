# -*- coding: utf-8 -*-
"""
Created on @Time:2019/2/15 15:44
...
tf.flags.DEFINE_float("learning_rate_decay_factor", 0.9,
                      "Learning rate decays by this much.")
tf.flags.DEFINE_boolean("train", False, "Whether train the model")
...
tf.flags.DEFINE_integer("size", 128, "Size of each model layer.")
tf.flags.DEFINE_integer("word_embedding_size", 128, "word embedding size")
tf.flags.DEFINE_integer("num_layers", 2, "Number of layers in the model.")
tf.flags.DEFINE_integer("in_vocab_size", 244, "max vocab Size.")
tf.flags.DEFINE_integer("out_vocab_size", 28, "max tag vocab Size.")
tf.flags.DEFINE_string("data_dir", "data/air_conditional", "Data directory")
tf.flags.DEFINE_string("train_dir", "model_tmp", "Training directory.")
tf.flags.DEFINE_integer("max_train_data_size", 0, "Limit on the size of training data (0: no limit)")
tf.flags.DEFINE_integer("steps_per_checkpoint", 100, "How many training steps to do per checkpoint.")
tf.flags.DEFINE_integer("max_training_steps", 30000, "Max training steps.")
tf.flags.DEFINE_integer("max_test_data_size", 0, "Max size of test set.")
tf.flags.DEFINE_boolean("use_attention", True, "Use attention based RNN")
tf.flags.DEFINE_integer("max_sequence_length", 15, "Max sequence length.")
tf.flags.DEFINE_float("dropout_keep_prob", 0.5, "dropout keep cell input and output prob.")
tf.flags.DEFINE_boolean("bidirectional_rnn", True, "Use birectional RNN")
tf.flags.DEFINE_string("task", "joint", "Options: joint; intent; tagging")
...
def get_perf(filename):
    ...
    stdout, _ = proc.communicate(''.join(open(filename, 'r', encoding='UTF-8').readlines()).encode('utf-8'))
            ...
            if current_step % FLAGS.steps_per_checkpoint == 0:
                ...
                if task['tagging'] == 1 \
                    ...
                    subprocess.call(['mv',
                                     ...
                                     % best_test_score], shell=True)


def evaluate_line():
    data_dir = FLAGS.data_dir
    in_vocab_size = FLAGS.in_vocab_size
    out_vocab_size = FLAGS.out_vocab_size
    encoder_size = FLAGS.max_sequence_length

    # Create vocabularies of the appropriate sizes.
    vocab_path = os.path.join(data_dir, "in_vocab_%d.txt" % in_vocab_size)
    tag_vocab_path = os.path.join(data_dir, "out_vocab_%d.txt" % out_vocab_size)
    ...
    with tf.Session(config=config) as sess:
        ...
        while True:
            ...
            encoder_inputs = [np.array(_) for _ in encoder_input + encoder_pad]

            input_feed = {model.sequence_length: np.array([len(encoder_input)], dtype=np.int32)}
            for l in range(encoder_size):
                input_feed[model.encoder_inputs[l].name] = [encoder_inputs[l]]

            output_feed = []
            for i in range(len(encoder_input)):
                output_feed.append(model.tagging_output[i])
            # output_feed.append(model.tagging_output)
            output_feed.append(model.classification_output)
